...
"""
GoogLeNet makes use of inception modules, which are blocks that contain convolutional layers of
different size strides, as well as a max pooling layer. The idea behind this is dimensionality reduction,
as that supports speed for training. With these different sized convolution layers, features can be extracted
from the images at different depths, eventually concatenating these in a depth concatenation layer due to all
convolutional layers returning images of the same height and width.
""";

# Model itself (GoogLeNet is also called Inception)
...
# Each Convolution block with Batch Norm and ReLU activation
...
# The inception block (module) itself
